...
int socklisten()
{
    ...
    while(1)
    {
        ...
        {
        	int x = errno;
            Log(LOGL_ERROR, LOGT_NETWORK, "Client sockfd error: %d", x);
        }
...
*/        
        for (int i = 0; i < 20; i++)
		{
            ...
            Log(LOGL_DEBUG, LOGT_NETWORK, "Send char %c ", pp);
        ...
        if(fork() == 0)
        {
            ...
            Log(LOGL_DEBUG, LOGT_NETWORK, "Client send: %c", ch);
            printf("Received: %c", ch);
